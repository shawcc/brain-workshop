...
- **路由**：React Router，覆盖算法课首页、游戏详情、挑战页、算法实验室、学习档案、家长/老师入口。
- **状态管理**：优先使用 React Context 和自定义 Hooks；复杂编辑状态使用 Zustand。
- **游戏运行时**：前端配置驱动渲染，首版承载迷宫最短路 BFS、汉诺塔递归、数独回溯三类经典算法课。
- **后端服务**：Supabase Auth + Supabase PostgreSQL + Supabase Storage。
- **部署**：GitHub 托管代码，Vercel 连接 GitHub 自动预览和生产部署。
- **初始化工具**：Vite。
- **算法教练首版**：前端课程清晰度检查 + Supabase 保存建议记录；保留 `/api/guide` Serverless API 扩展点，后续可接入 AI 模型。

## 3. 路由定义
| 路由 | 用途 |
|------|------|
| `/` | 算法课首页与经典游戏库 |
| `/games/:gameId` | 算法游戏详情页 |
| `/play/:gameId` | 算法挑战页 |
| `/create` | 新建算法实验 |
| `/create/:draftId` | 编辑已有算法实验 |
| `/remix/:gameId` | 基于课程生成算法实验 |
| `/me` | 学习档案 |
| `/auth` | 家长/老师入口 |

## 4. API 定义
首版优先通过 Supabase 客户端直接访问数据库，并用 RLS 控制权限。Vercel Serverless API 仅保留算法教练扩展点。
...
## 8. 首版实现策略
- 先用本地 Mock 数据完成算法课首页、详情页、挑战页、算法实验室和学习档案，保证产品可演示。
- 再接入 Supabase Auth 与数据库，实现真实账号、学习进度、算法实验、收藏课程。
- 算法教练先采用确定性规则引擎，避免首版依赖未确定的 AI 服务。
- 保留清晰的数据模型和 API 边界，使后续扩展个性化算法讲解、班级管理和课程审核更容易。
