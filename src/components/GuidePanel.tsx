  ...
  return (
    <aside className="rounded-[2rem] border border-cyan/20 bg-cyan/10 p-5 shadow-2xl shadow-cyan/5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.24em] text-cyan">关卡教练</p>
          <h2 className="mt-2 font-display text-3xl font-black">好玩清晰度 {report.score}</h2>
        </div>
      ...
      <div className="mt-5 rounded-2xl border border-cream/10 bg-ink/40 p-4">
        <div className="flex items-center gap-2 text-sm font-bold">
          ...
          {report.publishReady ? "适合给孩子玩" : "建议继续打磨"}
        </div>
        ...
        {report.issues.length === 0 ? (
          <div className="flex gap-3 rounded-2xl border border-cream/10 bg-cream/8 p-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-cream/62">玩法目标、最佳解法和玩完后的讲解都已经清楚。</p>
            </div>
          </div>
